/* tslint:disable */
/* eslint-disable */
// Generated by Microsoft Kiota
// @ts-ignore
import { OffersRequestBuilderNavigationMetadata, OffersRequestBuilderRequestsMetadata, type OffersRequestBuilder } from './offers/index.js';
// @ts-ignore
import { TextTranslationRequestBuilderRequestsMetadata, type TextTranslationRequestBuilder } from './textTranslation/index.js';
// @ts-ignore
import { type UsersRequestBuilder, UsersRequestBuilderNavigationMetadata, UsersRequestBuilderRequestsMetadata } from './users/index.js';
// @ts-ignore
import { type BaseRequestBuilder, type KeysToExcludeForNavigationMetadata, type NavigationMetadata } from '@microsoft/kiota-abstractions';

/**
 * Builds and executes requests for operations under /api/v1
 */
export interface V1RequestBuilder extends BaseRequestBuilder<V1RequestBuilder> {
    /**
     * The offers property
     */
    get offers(): OffersRequestBuilder;
    /**
     * The textTranslation property
     */
    get textTranslation(): TextTranslationRequestBuilder;
    /**
     * The users property
     */
    get users(): UsersRequestBuilder;
}
/**
 * Uri template for the request builder.
 */
export const V1RequestBuilderUriTemplate = "{+baseurl}/api/v1";
/**
 * Metadata for all the navigation properties in the request builder.
 */
export const V1RequestBuilderNavigationMetadata: Record<Exclude<keyof V1RequestBuilder, KeysToExcludeForNavigationMetadata>, NavigationMetadata> = {
    offers: {
        requestsMetadata: OffersRequestBuilderRequestsMetadata,
        navigationMetadata: OffersRequestBuilderNavigationMetadata,
    },
    textTranslation: {
        requestsMetadata: TextTranslationRequestBuilderRequestsMetadata,
    },
    users: {
        requestsMetadata: UsersRequestBuilderRequestsMetadata,
        navigationMetadata: UsersRequestBuilderNavigationMetadata,
    },
};
/* tslint:enable */
/* eslint-enable */
